...
from ..model.modules import HuggingfaceTextEncoder
...
class BreastClip(nn.Module):
    ...
    def encode_text(self, text_tokens):
        # For ModernBERT, we need to pass the input_ids and attention_mask
        if isinstance(self.text_encoder, torch.nn.parallel.DistributedDataParallel):
            text_encoder = self.text_encoder.module
        else:
            text_encoder = self.text_encoder
            
        if isinstance(text_encoder, HuggingfaceTextEncoder):
            # For HuggingfaceTextEncoder
            ...
            text_features = outputs.hidden_states[-1]  # Get the last hidden state
