...
def do_experiements(args, device):
    ...
    if args.dataset.lower() == 'vindr':
        ...
        if 'fold' in args.df.columns and args.df['fold'].nunique() > 1:
            args.train_folds = args.df[args.df['fold'] != args.cur_fold].reset_index(drop=True)
            args.valid_folds = args.df[args.df['fold'] == args.cur_fold].reset_index(drop=True)
        elif 'split' in args.df.columns:
            args.train_folds = args.df[args.df['split'].isin(['train', 'training'])].reset_index(drop=True)
            args.valid_folds = args.df[args.df['split'].isin(['val', 'valid', 'validation', 'test'])].reset_index(drop=True)
        else:
            raise ValueError("Custom dataset needs either a 'fold' column or a 'split' column.")
