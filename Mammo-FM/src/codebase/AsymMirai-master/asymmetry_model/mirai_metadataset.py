...
exam_record = namedtuple('exam_foo',
                         ['eid', 'label', 'l_cc_img', 'l_cc_path', 'r_cc_img', 'r_cc_path', 'l_mlo_img', 'l_mlo_path',
...
class MiraiMetadataset(Dataset):
    ...
    def __init__(
            self, metadata_frame, allow_incomplete=False, verbose=False, resizer=None, mode="training",
            load_images_async=True, align_images=True, oversample_cancer_rate=None, multiple_pairs_per_exam=False,
            label_col="cancer1yr_updated", dataset="bu"):
        """
        ...
        self.bu_path = "/restricted/projectnb/batmanlab/shared/Data/RSNA_Breast_Imaging/Dataset/External/BU_Mammo/png"
        self.mode = mode
    ...
    def load_img_path(self, path):
        ...
        if self.dataset.lower() == "rsna":
            ...
            if "controls" in img_path:
                img_path = Path(self.bu_path) / "controls" / exam_id / image_name
            elif "cases" in img_path:
                img_path = Path(self.bu_path) / "cases" / exam_id / image_name
    ...
    def load_img(self, path):
        ...
        img = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)  # .astype('int32')
    ...
    async def load_imgs_async(self, paths):
        ...
        coroutines = [self.load_img_async(self.load_img_path(path)) for path in paths]
