...
class LocalizedDifModel(torch.nn.Module):
    ...
    def __init__(
        ...
        if embedding_model is None:
            ...
            mirai = extract_mirai_backbone(chk_pt)
            print("==================================== MIRAI Model ====================================")
            print(mirai)
            ...
            if count <= 1:
                ...
                self.backbone = self.backbone.to("cuda:0" if count == 1 else "cpu")
            else:
    ...
    def forward(self, left_cc, right_cc, left_mlo, right_mlo, exam_list=None, **kwargs):
        ...
        for left, right, view in exam_list:
            ...
            # print(left.size()) # torch.Size([3, 3, 1664, 2048])
            # print(right.size()) # torch.Size([3, 3, 1664, 2048])
            # print(left_embedding.size()) # torch.Size([3, 512, 52, 64])
            # print(right_embedding.size()) # torch.Size([3, 512, 52, 64])
            # print(xxx)
        ...
        # print(pred)
        # print(torch.stack([1 - pred, pred], dim=1))
        # print(xxx)
